...
//|        [v3.7] ティックを追跡するため、GOLDmicroのチャートにすること |
...
//|     - ea_monitor_events_v37.csv: [v3.7] 初弾・ナンピン・決済の要約   |
...
#property description "EA_Monitor v3.7: 監視・記録 + 軽量イベント記録 + 月末月初の自動停止 + 足替わりタイマー"
...
extern bool   EnableEventLog       = true;                         // ea_monitor_events_v37.csv を出力するか
extern string EventFileName        = "ea_monitor_events_v37.csv";
...
int OnInit()
{
   ...
   if(EnableEventLog)
      WriteHeaderIfNeeded(EventFileName, EventHeader());
   ...
   Print("EA_Monitor v3.7 起動: 監視間隔=", PollSeconds, "秒, TargetMagics=", TargetMagics,
         ", TargetSymbols=", (TargetSymbols=="" ? "(全通貨)" : TargetSymbols),
         ", 指値監視=", TrackPendingOrders, ", 特徴量ログ=", EnableFeatureLog, "(", FeatureBaseTF, ")",
         ", 1分状態ログ=", EnableM1StateLog, ", イベント記録=", EnableEventLog, ", 月末月初停止=", EnableMonthEndStop,
         ", 自動売買OFF=", AutoTradingOffOnStop, ", DLL許可=", IsDllsAllowed());
   Print("EA_Monitor v3.7: ", NextStopWindowText());
...
void EvWrite(string &f[])
{
   string row = f[0];
   for(int i = 1; i < EV_NCOLS; i++) row += ";" + f[i];
   AppendLine(EventFileName, row);
}
...
void SendStartupNotifyOnce()
{
   ...
   string msg = "[EA_Monitor v3.7] 起動しました\n"
