...
//|   v3.6: ナンピン束をコメント(例: b_3 = ロジックb の4段目)で判定。    |
//|         同じ方向のa〜eが混ざって段数がずれていた不具合を修正         |
//|   v3.7: ラグナロクの軽量イベント記録(ea_monitor_events_v37.csv)。    |
//|         ナンピン時: 直前レグ以降の高値・安値、最大逆行と戻り幅、       |
//|                     時間と値幅の条件が揃ってからの待ち時間          |
//|         決済時:     最後のレグ以降の最大含み益(MFE)と時刻、          |
//|                     含み益が100ptsに達した時刻とその後の最小含み益   |
//|         初弾時:     3/11/20分前・15分足2本前の終値との差、             |
//|                     直近5/15/60分の高値・安値                        |
//|         値動きはティックごと(OnTick+1秒タイマー)に追跡する           |
//|   v3.8: (1) 口座の種類(リアル/デモ)の切り替え。デモはファイル名の    |
//|             頭に demo_ を付け、月末月初の停止とTelegramを使わない    |
//|         (2) イベント記録にM1/M5/M15のRSI・ストキャス(形成中の足と    |
//|             確定足)を追加(ea_monitor_events_v38.csv)                 |
//|         (3) ナンピン待機中の記録(ea_monitor_nanpin_wait_v38.csv)      |
//|         (4) 起動時に口座履歴を書き出す(ea_monitor_history_v38.csv)   |
//|                                                                  |
//|   使い方:                                                         |
//|     1. 対象EAを通常どおりチャートに設定して稼働させる               |
//|     2. 別の空きチャートにこのEAを1本だけ設定する                    |
//|        [v3.7] ティックを追跡するため、Ragnarokが取引する銘柄のチャートにすること |
...
//|     - ea_monitor_events_v38.csv: [v3.8] 初弾・ナンピン・決済の要約    |
//|                                  (v3.7の列+オシレーター)           |
//|     - ea_monitor_nanpin_wait_v38.csv: [v3.8] ナンピン待機中の記録   |
//|     - ea_monitor_history_v38.csv: [v3.8] 起動時の口座履歴(上書き)   |
//|     [v3.8] デモ口座を選ぶと、全ファイル名の頭に demo_ が付く         |
...
#property description "EA_Monitor v3.8: 監視・記録 + 軽量イベント記録(オシレーター付き) + ナンピン待機の記録 + 口座履歴 + リアル/デモ切り替え + 月末月初の自動停止 + 足替わりタイマー"
...
extern ENUM_ACCOUNT_MODE AccountMode   = ACC_REAL; // 口座の種類(デモはファイル名の頭に DemoFilePrefix を付ける)
extern string DemoFilePrefix           = "demo_";  // デモ口座のファイル名の頭に付ける文字
extern bool   TelegramInDemo           = false;    // デモ口座でもTelegram通知するか(既定: しない)
...
extern string BasketFileName        = "ea_monitor_baskets_v36.csv"; // ナンピン束ごとの集計([v3.6] logic列を追加したため新ファイル)
...
extern double NanpinMinPts         = 110;                          // ナンピンの値幅条件(記録用。解析19)
extern double ProbeProfitPts       = 100;                          // 含み益がこの幅(pts)に達した時刻を記録(早期決済の調査用)

//--- [v3.8] ナンピン待機中の記録と口座履歴の書き出し -------------------------------
//  待機中の記録: 時間と値幅の条件が揃ったのにナンピンが入っていない間だけ、WaitLogSeconds 秒ごとに1行
//  口座履歴: 起動時に1回、ラグナロクのマジックの過去の全取引と今の建玉を書き出す(ファイルは上書き)
//           MT4の「口座履歴」タブで「全期間」を表示しておくこと
extern bool   EnableWaitLog        = true;                              // ea_monitor_nanpin_wait_v38.csv を出力するか
extern string WaitFileName         = "ea_monitor_nanpin_wait_v38.csv";
extern int    WaitLogSeconds       = 10;                                // 待機中の記録の間隔(秒)
extern bool   EnableHistoryDump    = true;                              // 起動時に ea_monitor_history_v38.csv を書き出すか
extern string HistoryFileName      = "ea_monitor_history_v38.csv";
...
extern ENUM_DISP_CORNER DisplayCorner     = DISP_RIGHT_UPPER; // 表示位置
extern int              DisplayX          = 10;               // 表示位置: 横(角からのピクセル)
extern int              DisplayY          = 30;               // 表示位置: 縦(角からのピクセル)
extern int              DisplayGapPx      = 10;               // タイマーと稼働表示の間隔(ピクセル)
extern string           DisplayFont       = "MS UI Gothic";   // 文字の種類
extern int              TimerFontSize     = 30;               // 足替わりタイマーの文字サイズ
...
struct BasketState
{
   ...
   int      cntGridStepPts;
   string   grid;            // [v3.6] コメントの頭文字(ラグナロクのロジック a〜e)。コメントが無い注文は空
   int      lastLegNo;       // [v3.6] 最後に追加した段のコメント番号(b_3 なら 3)。番号が無い注文は -1
   // [v3.7] 最後のレグ以降の値動き(ティックごとに更新。新しいレグが入ると初期化)
   datetime lastLegTime;     // 最後のレグの建玉時刻
   double   advMaxPts;       // 最後のレグの建値からの最大逆行(pts。買いはask、売りはbidで測る)
   datetime advMaxTime;
   double   highSince;       // 最後のレグ以降のbidの高値
   double   lowSince;        // 最後のレグ以降のbidの安値
   datetime condMetTime;     // 時間と値幅の条件(NanpinMinSeconds/NanpinMinPts)が初めて揃った時刻(0=未達)
   double   mfePts;          // 平均建値からの最大含み益(pts。買いはbid、売りはaskで測る)
   datetime mfeTime;
   double   maePts;          // 平均建値からの最大含み損(pts。正の値)
   datetime probeTime;       // 含み益が ProbeProfitPts に初めて達した時刻(0=未達)
   double   minFavAfterProbe;// その後の最小含み益(pts)
   bool     restored;        // 起動時に復元した束(起動前の値動きは含まない)
   datetime lastWaitLog;     // [v3.8] ナンピン待機中の記録を最後に書いた時刻
};
...
//--- [v3.7] ロジック・方向ごとの直近の初弾・決済の時刻(イベント記録用) -------------
string       g_evKey[];
datetime     g_evLastEntry[];
datetime     g_evLastClose[];
bool         g_restoring = false;   // 起動時の束の復元中
...
//--- [v3.8] 実際に使うファイル名(デモ口座では頭に DemoFilePrefix が付く) ---------------
string       g_fnLog, g_fnEquity, g_fnBasket, g_fnFeature, g_fnM1State, g_fnEvent, g_fnWait, g_fnHistory;
...
int OnInit()
{
   // [v3.8] 口座の種類の確認。選んだ種類と実際の口座が違う時は起動しない(本番のファイルを上書きしないため)
   if(IsDemo() != IsDemoMode())
   {
      string warn = "EA_Monitor v3.8: 口座の種類の設定(" + (IsDemoMode() ? "デモ" : "リアル")
                  + ")と、ログイン中の口座(" + (IsDemo() ? "デモ" : "リアル") + ")が違います。"
                  + "設定の「口座の種類」を直してください。";
      Print(warn);
      Alert(warn);
      return(INIT_PARAMETERS_INCORRECT);
   }
   g_fnLog      = FileNameFor(LogFileName);
   g_fnEquity   = FileNameFor(EquityFileName);
   g_fnBasket   = FileNameFor(BasketFileName);
   g_fnFeature  = FileNameFor(FeatureFileName);
   g_fnM1State  = FileNameFor(M1StateFileName);
   g_fnEvent    = FileNameFor(EventFileName);
   g_fnWait     = FileNameFor(WaitFileName);
   g_fnHistory  = FileNameFor(HistoryFileName);
   ...
   WriteHeaderIfNeeded(g_fnEquity,
   ...
   WriteHeaderIfNeeded(g_fnBasket,
      ...
      "peak_floating_profit;worst_floating_drawdown;logic");
   ...
   if(EnableFeatureLog)
      WriteHeaderIfNeeded(g_fnFeature,
   ...
   if(EnableM1StateLog)
      WriteHeaderIfNeeded(g_fnM1State, M1StateHeader());
   if(EnableEventLog)
      WriteHeaderIfNeeded(g_fnEvent, EventHeader());
   if(EnableEventLog && EnableWaitLog)
      WriteHeaderIfNeeded(g_fnWait, WaitHeader());
   if(EnableHistoryDump)
      DumpAccountHistory();
   ...
   Print("EA_Monitor v3.8 起動: 口座=", AccountNumber(), (IsDemoMode() ? "(デモ)" : "(リアル)"),
         ", ファイル名の例=", g_fnEvent, ", 監視間隔=", PollSeconds, "秒, TargetMagics=", TargetMagics,
         ", TargetSymbols=", (TargetSymbols=="" ? "(全通貨)" : TargetSymbols),
         ", 指値監視=", TrackPendingOrders, ", 特徴量ログ=", EnableFeatureLog, "(", FeatureBaseTF, ")",
         ", 1分状態ログ=", EnableM1StateLog, ", イベント記録=", EnableEventLog, ", 月末月初停止=", MonthEndStopActive(), ", Telegram=", TelegramActive(),
         ", 自動売買OFF=", AutoTradingOffOnStop, ", DLL許可=", IsDllsAllowed());
   Print("EA_Monitor v3.8: ", NextStopWindowText());
...
void OnTimer()
{
   SyncServerTime();
   FlushPendingLines();
   if(EnableEventLog) TrackBasketTicks();   // [v3.7] 決済・ナンピンを検知する前に最新の値動きを反映
   ScanForEvents();
...
{
   SyncServerTime();
   if(EnableEventLog) TrackBasketTicks();   // [v3.7] ティックごとに高値・安値・含み益を追跡
   UpdateDisplay();
}
...
// [v3.8] 口座の種類
bool IsDemoMode() { return (AccountMode == ACC_DEMO); }
bool TelegramActive() { return EnableTelegram && (!IsDemoMode() || TelegramInDemo); }
bool MonthEndStopActive() { return EnableMonthEndStop && !IsDemoMode(); }

// [v3.8] デモ口座ではファイル名の頭に DemoFilePrefix を付ける(リアルは今までと同じ名前)
string FileNameFor(string name)
{
   return IsDemoMode() ? DemoFilePrefix + name : name;
}
...
void RebuildBasketsFromSnapshot()
{
   ...
   ArrayResize(g_legTicket, 0);
   ArrayResize(g_legBasketId, 0);
   // [v3.6] 段の順番(b_0 → b_1 → …)どおりに束へ入れるため、建玉時刻の古い順に処理する
   int n = ArraySize(g_prevOrders);
   g_restoring = true;   // [v3.7] 復元した束は、起動前の値動きを含まない印を付ける
   bool used[];
   ArrayResize(used, n);
   ...
   for(int k = 0; k < n; k++)
   {
      ...
      for(int i = 0; i < n; i++)
      {
         if(used[i] || g_prevOrders[i].type > OP_SELL) continue; // 成行のみ
         if(pick < 0 || g_prevOrders[i].openTime < g_prevOrders[pick].openTime
            || (g_prevOrders[i].openTime == g_prevOrders[pick].openTime && g_prevOrders[i].ticket < g_prevOrders[pick].ticket))
            pick = i;
      }
      if(pick < 0) break;
      used[pick] = true;
      ApplyLegToBasket(g_prevOrders[pick], false);  // false=ログ出力なしで束情報だけ更新
   }
   g_restoring = false;
}
...
//+------------------------------------------------------------------+
//| バスケット検索/新規作成                                           |
//|   [v3.6] ラグナロクは a〜e の各ロジックが買い・売りを別々に持ち、    |
//|   同じ方向で複数の束を同時に持つこともある。v3.5までは               |
//|   「通貨ペア+マジック+方向」だけで束を分けていたため、同じ方向の     |
//|   a〜eが1つの束に混ざり、段数・合計ロット・平均建値がずれていた。    |
//|   v3.6ではコメント(例: b_3)を読み、                                 |
//|     ・頭文字(b) = ロジック、番号(3) = 何段目か(0始まり)             |
//|     ・番号0 = 新しい束の初弾(同じロジック・方向の束があっても別の束)  |
//|     ・番号k = 同じロジック・方向で、最後の段がk-1の束に追加           |
//|   で判定する。コメントが「英字_数字」でない注文は従来どおり。        |
//+------------------------------------------------------------------+
// コメント "b_3" → grid="b", no=3。形式が違えば false
bool ParseLegComment(string comment, string &grid, int &no)
{
   grid = ""; no = -1;
   int us = StringFind(comment, "_");
   if(us <= 0 || us >= StringLen(comment) - 1) return false;
   ...
   for(int j = 0; j < StringLen(head); j++)
   {
      ushort ch2 = StringGetCharacter(head, j);
      if(!((ch2 >= 'a' && ch2 <= 'z') || (ch2 >= 'A' && ch2 <= 'Z'))) return false;
   }
   grid = head;
   no   = (int)StringToInteger(tail);
   return true;
}

// 同じ通貨ペア・マジック・方向・ロジックの束のうち、最も新しいもの
int FindBasketIndex(string sym, int magic, int type, string grid)
{
   int found = -1;
   for(int i = 0; i < ArraySize(g_baskets); i++)
      if(g_baskets[i].symbol == sym && g_baskets[i].magic == magic && g_baskets[i].type == type
         && g_baskets[i].grid == grid)
         if(found < 0 || g_baskets[i].basketId > g_baskets[found].basketId) found = i;
   return found;
}

// ナンピン(番号k)の追加先: 最後の段がk-1の束。複数あれば直前段から最も逆行している束
int FindBasketForNanpin(string sym, int magic, int type, string grid, int no, double price)
{
   int best = -1; double bestAdv = -1e18;
   for(int i = 0; i < ArraySize(g_baskets); i++)
   {
      if(g_baskets[i].symbol != sym || g_baskets[i].magic != magic || g_baskets[i].type != type) continue;
      if(g_baskets[i].grid != grid || g_baskets[i].lastLegNo != no - 1) continue;
      double adv = (type == OP_BUY || type == OP_BUYLIMIT || type == OP_BUYSTOP)
                   ? g_baskets[i].lastPrice - price : price - g_baskets[i].lastPrice;
      if(best < 0 || adv > bestAdv) { best = i; bestAdv = adv; }
   }
   if(best < 0) best = FindBasketIndex(sym, magic, type, grid); // 途中の段を取りこぼした場合など
   return best;
}

int FindBasketIndexById(int basketId)
{
   for(int i = 0; i < ArraySize(g_baskets); i++)
      if(g_baskets[i].basketId == basketId) return i;
   return -1;
}

void RegisterLegTicket(int ticket, int basketId)
{
   for(int i = 0; i < ArraySize(g_legTicket); i++)
      if(g_legTicket[i] == ticket) { g_legBasketId[i] = basketId; return; }
   int n = ArraySize(g_legTicket);
   ArrayResize(g_legTicket, n + 1); ArrayResize(g_legBasketId, n + 1);
   g_legTicket[n] = ticket; g_legBasketId[n] = basketId;
}

int BasketIdOfTicket(int ticket)
{
   for(int i = 0; i < ArraySize(g_legTicket); i++)
      if(g_legTicket[i] == ticket) return g_legBasketId[i];
   return 0;
}

void UnregisterLegTicket(int ticket)
{
   int n = ArraySize(g_legTicket);
   for(int i = 0; i < n; i++)
      if(g_legTicket[i] == ticket)
      {
         g_legTicket[i] = g_legTicket[n-1]; g_legBasketId[i] = g_legBasketId[n-1];
         ArrayResize(g_legTicket, n - 1); ArrayResize(g_legBasketId, n - 1);
         return;
      }
}

int CreateBasket(string sym, int magic, int type, string grid)
{
   ...
   g_baskets[n].grid             = grid;
   g_baskets[n].lastLegNo        = -1;
   g_baskets[n].restored         = false;
   ResetLegTracking(n, 0);
   return n;
}
...
{
   string grid; int legNo;
   bool hasNo = ParseLegComment(o.comment, grid, legNo);
   int bi = -1;
   if(hasNo)
   {
      if(legNo > 0) bi = FindBasketForNanpin(o.symbol, o.magic, o.type, grid, legNo, o.openPrice);
      // 番号0は常に新しい束(同じロジック・方向の束が残っていても別の束として扱う)
   }
   else
      bi = FindBasketIndex(o.symbol, o.magic, o.type, "");
   if(bi < 0) bi = CreateBasket(o.symbol, o.magic, o.type, grid);
   ...
   g_baskets[bi].lastLegNo = hasNo ? legNo : g_baskets[bi].legCount - 1;
   RegisterLegTicket(o.ticket, g_baskets[bi].basketId);
   ResetLegTracking(bi, o.openTime);                 // [v3.7] 最後のレグ以降の値動きを測り直す
   if(g_restoring) g_baskets[bi].restored = true;

   basketIdOut  = g_baskets[bi].basketId;
   legIndexOut  = hasNo ? legNo + 1 : g_baskets[bi].legCount; // 何段目か(コメント b_3 → 4段目)
...
//+------------------------------------------------------------------+
bool ApplyCloseToBasket(OrderSnap &o, double realizedPnl, BasketState &closedSummary, int &basketIdOut)
{
   basketIdOut = 0;
   int bi = FindBasketIndexById(BasketIdOfTicket(o.ticket));
   if(bi < 0)
   {
      string grid; int legNo;
      ParseLegComment(o.comment, grid, legNo);
      bi = FindBasketIndex(o.symbol, o.magic, o.type, grid);
   }
   UnregisterLegTicket(o.ticket);
...
{
   int nb = ArraySize(g_baskets);
   if(nb == 0) return;
   double floating[];
   ArrayResize(floating, nb);
   ArrayInitialize(floating, 0.0);
   int total = OrdersTotal();
   for(int j = 0; j < total; j++)
   {
      if(!OrderSelect(j, SELECT_BY_POS, MODE_TRADES)) continue;
      if(OrderType() > OP_SELL) continue;
      int bi = FindBasketIndexById(BasketIdOfTicket(OrderTicket())); // [v3.6] 束はチケットで特定
      if(bi < 0) continue;
      floating[bi] += OrderProfit() + OrderSwap() + OrderCommission();
   }
   for(int i = 0; i < nb; i++)
   {
      if(floating[i] > g_baskets[i].peakFloating)  g_baskets[i].peakFloating  = floating[i];
      if(floating[i] < g_baskets[i].worstFloating) g_baskets[i].worstFloating = floating[i];
   }
}
...
void LogLifecycleEvent(string event, OrderSnap &o, double closePriceOverride, double realizedPnlOverride)
{
   ...
   {
      // [v3.7] ナンピンは、束を更新する前に「直前のレグ以降の値動き」を控えておく
      string eg = ""; int en = -1;
      bool evTarget = EnableEventLog && IsRagnarokMagic(o.magic) && ParseLegComment(o.comment, eg, en);
      BasketState pre;
      bool hasPre = false;
      if(evTarget && en > 0)
      {
         TrackBasketTicks();
         int pbi = FindBasketForNanpin(o.symbol, o.magic, o.type, eg, en, o.openPrice);
         if(pbi >= 0) { pre = g_baskets[pbi]; hasPre = true; }
      }
      ...
      if(evTarget)
      {
         if(en == 0) WriteEntryEvent(o, eg, basketId);
         else        WriteNanpinEvent(o, eg, en, basketId, pre, hasPre, lotMult);
      }
   }
   else if(event == "CLOSE" || event == "CLOSE_NOHIST")
   {
      BasketState closedSummary;
      int closedBasketId = 0;
      bool basketClosed = ApplyCloseToBasket(o, realizedPnlOverride, closedSummary, closedBasketId);
      int bi = basketClosed ? -1 : FindBasketIndexById(closedBasketId);
      if(bi >= 0)
      {
      ...
      else if(basketClosed)
      {
         ...
         if(EnableEventLog && IsRagnarokMagic(o.magic) && closedSummary.grid != "")
            WriteBasketCloseEvent(closedSummary, closePriceOverride, event);   // [v3.7]
      }
      string cg; int cn;
      if(ParseLegComment(o.comment, cg, cn)) legIndex = cn + 1; // 決済した段(コメントの番号から)
   }
                             ...
                             int basketId, int legIndex, int legs, double totalLots, double avgPrice)
{
   ...
   if(event == "OPEN" || event == "FILL")
   {
      ...
      string lg; int ln;
      ParseLegComment(o.comment, lg, ln);
      string legLabel = (legIndex <= 1) ? "新規エントリー" : "ナンピン追加(" + IntegerToString(legIndex) + "段目)";
      msg = "[EA_Monitor] " + LogicLabel(lg, o.type) + " " + legLabel + "\n"
   ...
   else if(event == "CLOSE" || event == "CLOSE_NOHIST")
   {
      ...
      string cg2; int cn2;
      ParseLegComment(o.comment, cg2, cn2);
      msg = "[EA_Monitor] " + LogicLabel(cg2, o.type) + " " + (event == "CLOSE" ? "決済" : "決済(履歴が取れず損益不明)")
          + (cn2 >= 0 ? " " + IntegerToString(cn2 + 1) + "段目" : "") + "\n"
...
//+------------------------------------------------------------------+
//| [v3.7] ラグナロクの軽量イベント記録                                 |
//|   ea_monitor_events_v37.csv に、初弾(ENTRY)・ナンピン(NANPIN)・       |
//|   バスケットの決済(BASKET_CLOSE)を1行ずつ書く。                     |
//|   列は解析用の再構成ファイル(tools/ragnarok/prior_extremes.py)と同じ |
//+------------------------------------------------------------------+
enum EV_COL
{
   EV_EVENT, EV_TIME, EV_LOGIC, EV_SIDE, EV_BASKET, EV_LEG, EV_PRICE, EV_BID, EV_ASK, EV_SPREAD,
   EV_MOM3, EV_MOM11, EV_MOM20, EV_MOM_M15C2, EV_PB_OPEN, EV_PB_HIGH, EV_PB_LOW, EV_PB_CLOSE,
   EV_HIGH5, EV_LOW5, EV_HIGH15, EV_LOW15, EV_HIGH60, EV_LOW60, EV_SEC_PREV_ENTRY, EV_SEC_PREV_CLOSE,
   EV_PREV_LEG_PRICE, EV_PREV_LEG_TIME, EV_SEC_PREV_LEG, EV_LOT_RATIO, EV_ADV_NOW, EV_ADV_MAX,
   EV_ADV_MAX_TIME, EV_HIGH_SINCE, EV_LOW_SINCE, EV_BOUNCE, EV_COND_TIME, EV_WAIT_SEC,
   EV_LEGS, EV_TOTAL_LOTS, EV_AVG_PRICE, EV_CLOSE_PRICE, EV_CLOSE_PTS, EV_PROFIT, EV_SEC_LAST_LEG,
   EV_SEC_FIRST_LEG, EV_MFE, EV_MFE_TIME, EV_SEC_MFE_TO_CLOSE, EV_MAE, EV_PROBE_TIME,
   EV_MIN_FAV_PROBE, EV_LAST_BID, EV_FAV_NOW, EV_GAP, EV_NCOLS
};

string EventHeader()
{
   return "event;time;logic;side;basket;leg;price;bid;ask;spread_pts;"
          ...
          "min_fav_after_reach100;last_bid;fav_now_pts;gap" + OscHeader();
}

// [v3.8] M1/M5/M15 の RSI(14) とストキャス(9,3,3)の%K・%D。0=形成中の足、1=確定足
string OscHeader()
{
   string h = "";
   string tfs[3] = {"m1", "m5", "m15"};
   for(int i = 0; i < 3; i++)
      h += ";" + tfs[i] + "_rsi0;" + tfs[i] + "_rsi1;" + tfs[i] + "_stoK0;" + tfs[i] + "_stoD0;"
         + tfs[i] + "_stoK1;" + tfs[i] + "_stoD1";
   return h;
}

string OscFields(string sym)
{
   string r = "";
   int tfs[3] = {PERIOD_M1, PERIOD_M5, PERIOD_M15};
   for(int i = 0; i < 3; i++)
      r += ";" + DoubleToString(iRSI(sym, tfs[i], 14, PRICE_CLOSE, 0), 2)
         + ";" + DoubleToString(iRSI(sym, tfs[i], 14, PRICE_CLOSE, 1), 2)
         + ";" + DoubleToString(iStochastic(sym, tfs[i], 9, 3, 3, MODE_SMA, 0, MODE_MAIN,   0), 2)
         + ";" + DoubleToString(iStochastic(sym, tfs[i], 9, 3, 3, MODE_SMA, 0, MODE_SIGNAL, 0), 2)
         + ";" + DoubleToString(iStochastic(sym, tfs[i], 9, 3, 3, MODE_SMA, 0, MODE_MAIN,   1), 2)
         + ";" + DoubleToString(iStochastic(sym, tfs[i], 9, 3, 3, MODE_SMA, 0, MODE_SIGNAL, 1), 2);
   return r;
}

bool IsRagnarokMagic(int magic)
{
   return (magic == RagnarokMagicBuy || magic == RagnarokMagicSell);
}

string EvTime(datetime t)
{
   return (t > 0) ? TimeToString(t, TIME_DATE|TIME_SECONDS) : "";
}

void EvInit(string &f[])
{
   ArrayResize(f, EV_NCOLS);
   for(int i = 0; i < EV_NCOLS; i++) f[i] = "";
}

void EvWrite(string &f[], string sym)
{
   string row = f[0];
   for(int i = 1; i < EV_NCOLS; i++) row += ";" + f[i];
   AppendLine(g_fnEvent, row + OscFields(sym));   // [v3.8] オシレーターを行末に付ける
}

// ロジック・方向ごとの直近の初弾・決済の時刻
int EvKeyIndex(string logic, int type)
{
   string key = logic + ((type == OP_BUY) ? "_buy" : "_sell");
   for(int i = 0; i < ArraySize(g_evKey); i++)
      if(g_evKey[i] == key) return i;
   int n = ArraySize(g_evKey);
   ArrayResize(g_evKey, n + 1); ArrayResize(g_evLastEntry, n + 1); ArrayResize(g_evLastClose, n + 1);
   g_evKey[n] = key; g_evLastEntry[n] = 0; g_evLastClose[n] = 0;
   return n;
}

// 新しいレグが入った時に、最後のレグ以降の値動きの記録を初期化する
void ResetLegTracking(int bi, datetime legTime)
{
   g_baskets[bi].lastLegTime      = legTime;
   g_baskets[bi].advMaxPts        = -1e18;
   g_baskets[bi].advMaxTime       = 0;
   g_baskets[bi].highSince        = 0;
   g_baskets[bi].lowSince         = 0;
   g_baskets[bi].condMetTime      = 0;
   g_baskets[bi].mfePts           = -1e18;
   g_baskets[bi].mfeTime          = 0;
   g_baskets[bi].maePts           = -1e18;
   g_baskets[bi].probeTime        = 0;
   g_baskets[bi].minFavAfterProbe = 0;
   g_baskets[bi].lastWaitLog      = 0;
}

// ティックごと(OnTick)と1秒ごと(OnTimer)に、進行中の束の値動きを更新する
void TrackBasketTicks()
{
   datetime now = TimeCurrent();
   for(int i = 0; i < ArraySize(g_baskets); i++)
   {
      if(g_baskets[i].legCount <= 0 || g_baskets[i].lastLegTime == 0) continue;
      if(g_baskets[i].type != OP_BUY && g_baskets[i].type != OP_SELL) continue;
      string sym = g_baskets[i].symbol;
      double bid = MarketInfo(sym, MODE_BID);
      double ask = MarketInfo(sym, MODE_ASK);
      double pt  = MarketInfo(sym, MODE_POINT);
      if(bid <= 0 || ask <= 0 || pt <= 0) continue;
      bool   isBuy = (g_baskets[i].type == OP_BUY);
      double adv = isBuy ? (g_baskets[i].lastPrice - ask) / pt : (bid - g_baskets[i].lastPrice) / pt;
      double fav = isBuy ? (bid - g_baskets[i].avgPrice) / pt : (g_baskets[i].avgPrice - ask) / pt;

      if(g_baskets[i].highSince == 0 || bid > g_baskets[i].highSince) g_baskets[i].highSince = bid;
      if(g_baskets[i].lowSince  == 0 || bid < g_baskets[i].lowSince)  g_baskets[i].lowSince  = bid;
      if(adv > g_baskets[i].advMaxPts) { g_baskets[i].advMaxPts = adv; g_baskets[i].advMaxTime = now; }
      if(g_baskets[i].condMetTime == 0 && now - g_baskets[i].lastLegTime >= NanpinMinSeconds && adv >= NanpinMinPts)
         g_baskets[i].condMetTime = now;
      if(fav > g_baskets[i].mfePts) { g_baskets[i].mfePts = fav; g_baskets[i].mfeTime = now; }
      if(-fav > g_baskets[i].maePts) g_baskets[i].maePts = -fav;
      if(g_baskets[i].probeTime == 0)
      {
         if(fav >= ProbeProfitPts) { g_baskets[i].probeTime = now; g_baskets[i].minFavAfterProbe = fav; }
      }
      else if(fav < g_baskets[i].minFavAfterProbe)
         g_baskets[i].minFavAfterProbe = fav;

      // [v3.8] 条件が揃ってからナンピンが入るまで、WaitLogSeconds 秒ごとに記録する
      if(EnableWaitLog && g_baskets[i].condMetTime > 0 && g_baskets[i].grid != ""
         && IsRagnarokMagic(g_baskets[i].magic)
         && now - g_baskets[i].lastWaitLog >= MathMax(1, WaitLogSeconds))
      {
         WriteWaitRow(i, now, bid, ask, adv);
         g_baskets[i].lastWaitLog = now;
      }
   }
}
...
string WaitHeader()
{
   return "time;account;logic;side;basket;legs;last_leg_price;last_leg_time;sec_since_last_leg;"
          "cond_met_time;sec_since_cond;bid;ask;adv_now_pts;adv_max_pts;bounce_pts;cond_now" + OscHeader();
}

// [v3.8] ナンピン待機中の1行: 今の逆行幅・最大逆行・戻り幅と、M1/M5/M15のオシレーター
void WriteWaitRow(int i, datetime now, double bid, double ask, double adv)
{
   string sym = g_baskets[i].symbol;
   int    dg  = (int)MarketInfo(sym, MODE_DIGITS);
   bool   condNow = (now - g_baskets[i].lastLegTime >= NanpinMinSeconds && adv >= NanpinMinPts);
   string row = EvTime(now) + ";"
              + IntegerToString(AccountNumber()) + ";"
              + g_baskets[i].grid + ";"
              + ((g_baskets[i].type == OP_BUY) ? "buy" : "sell") + ";"
              + IntegerToString(g_baskets[i].basketId) + ";"
              + IntegerToString(g_baskets[i].legCount) + ";"
              + DoubleToString(g_baskets[i].lastPrice, dg) + ";"
              + EvTime(g_baskets[i].lastLegTime) + ";"
              + IntegerToString((int)(now - g_baskets[i].lastLegTime)) + ";"
              + EvTime(g_baskets[i].condMetTime) + ";"
              + IntegerToString((int)(now - g_baskets[i].condMetTime)) + ";"
              + DoubleToString(bid, dg) + ";"
              + DoubleToString(ask, dg) + ";"
              + DoubleToString(adv, 1) + ";"
              + DoubleToString(g_baskets[i].advMaxPts, 1) + ";"
              + DoubleToString(g_baskets[i].advMaxPts - adv, 1) + ";"
              + (condNow ? "1" : "0")
              + OscFields(sym);
   AppendLine(g_fnWait, row);
}

// [v3.8] 起動時に1回、ラグナロクのマジックの過去の全取引と今の建玉を書き出す(上書き)
//  MT4の「口座履歴」タブに表示されている範囲だけが対象(「全期間」を選んでおくこと)
void DumpAccountHistory()
{
   int h = FileOpen(g_fnHistory, FILE_WRITE | FILE_ANSI | (UseCommonFolder ? FILE_COMMON : 0));
   if(h == INVALID_HANDLE) { Print("口座履歴の書き出し失敗(", g_fnHistory, ") err=", GetLastError()); return; }
   FileWriteString(h, "account;status;ticket;symbol;type;lots;open_time;open_price;close_time;close_price;"
                      "sl;tp;profit;swap;commission;magic;comment\r\n");
   int n = 0;
   for(int pass = 0; pass < 2; pass++)
   {
      int pool  = (pass == 0) ? MODE_HISTORY : MODE_TRADES;
      int total = (pass == 0) ? OrdersHistoryTotal() : OrdersTotal();
      for(int k = 0; k < total; k++)
      {
         if(!OrderSelect(k, SELECT_BY_POS, pool)) continue;
         if(OrderType() > OP_SELL || !IsRagnarokMagic(OrderMagicNumber())) continue;
         int dg = (int)MarketInfo(OrderSymbol(), MODE_DIGITS);
         if(dg <= 0) dg = 5;
         string row = IntegerToString(AccountNumber()) + ";"
                    + ((pass == 0) ? "closed" : "open") + ";"
                    + IntegerToString(OrderTicket()) + ";"
                    + OrderSymbol() + ";"
                    + OrderTypeStr(OrderType()) + ";"
                    + DoubleToString(OrderLots(), 2) + ";"
                    + EvTime(OrderOpenTime()) + ";"
                    + DoubleToString(OrderOpenPrice(), dg) + ";"
                    + ((pass == 0) ? EvTime(OrderCloseTime()) : "") + ";"
                    + ((pass == 0) ? DoubleToString(OrderClosePrice(), dg) : "") + ";"
   ...
   FileClose(h);
   Print("EA_Monitor v3.8: 口座履歴を書き出しました(", n, "件) → ", g_fnHistory);
}

// 初弾(_0): 3/11/20分前の終値との差、直前の足、直近5/15/60分の高値・安値
void WriteEntryEvent(OrderSnap &o, string logic, int basketId)
{
   string f[]; EvInit(f);
   datetime now = TimeCurrent();
   string sym = o.symbol;
   ...
   f[EV_EVENT]  = "ENTRY";
   f[EV_TIME]   = EvTime(now);
   f[EV_LOGIC]  = logic;
   f[EV_SIDE]   = (o.type == OP_BUY) ? "buy" : "sell";
   f[EV_BASKET] = IntegerToString(basketId);
   f[EV_LEG]    = "0";
   f[EV_PRICE]  = DoubleToString(o.openPrice, dg);
   f[EV_BID]    = DoubleToString(bid, dg);
   f[EV_ASK]    = DoubleToString(ask, dg);
   f[EV_SPREAD] = DoubleToString((ask - bid) / pt, 1);

   // 判定の足(今の足)の始値から m 分前に始まった足の終値との差(b=3分、c=11分、a=20分)
   int mins[3] = {3, 11, 20};
   int cols[3] = {EV_MOM3, EV_MOM11, EV_MOM20};
   datetime bar0 = iTime(sym, PERIOD_M1, 0);
   for(int k = 0; k < 3; k++)
   {
      int sh = iBarShift(sym, PERIOD_M1, bar0 - mins[k] * 60, false);
      if(sh < 0) continue;
      double c = iClose(sym, PERIOD_M1, sh);
      if(c > 0) f[cols[k]] = DoubleToString((bid - c) / pt, 1);
   }
   // aの方向ルール(解析20): 15分足の2本前の終値との差
   double c15 = iClose(sym, PERIOD_M15, 2);
   if(c15 > 0) f[EV_MOM_M15C2] = DoubleToString((bid - c15) / pt, 1);
   f[EV_PB_OPEN]  = DoubleToString(iOpen (sym, PERIOD_M1, 1), dg);
   f[EV_PB_HIGH]  = DoubleToString(iHigh (sym, PERIOD_M1, 1), dg);
   f[EV_PB_LOW]   = DoubleToString(iLow  (sym, PERIOD_M1, 1), dg);
   f[EV_PB_CLOSE] = DoubleToString(iClose(sym, PERIOD_M1, 1), dg);
   int wins[3]  = {5, 15, 60};
   int hcols[3] = {EV_HIGH5, EV_HIGH15, EV_HIGH60};
   int lcols[3] = {EV_LOW5, EV_LOW15, EV_LOW60};
   for(int w = 0; w < 3; w++)
   {
      int hi = iHighest(sym, PERIOD_M1, MODE_HIGH, wins[w], 1);
      int lo = iLowest (sym, PERIOD_M1, MODE_LOW,  wins[w], 1);
      if(hi >= 0) f[hcols[w]] = DoubleToString(iHigh(sym, PERIOD_M1, hi), dg);
      if(lo >= 0) f[lcols[w]] = DoubleToString(iLow (sym, PERIOD_M1, lo), dg);
   }
   int ki = EvKeyIndex(logic, o.type);
   if(g_evLastEntry[ki] > 0) f[EV_SEC_PREV_ENTRY] = IntegerToString((int)(o.openTime - g_evLastEntry[ki]));
   if(g_evLastClose[ki] > 0) f[EV_SEC_PREV_CLOSE] = IntegerToString((int)(o.openTime - g_evLastClose[ki]));
   g_evLastEntry[ki] = o.openTime;
   EvWrite(f, sym);
}

// ナンピン(_1以降): 直前レグ以降の高値・安値、最大逆行と戻り幅、条件が揃ってからの待ち時間
void WriteNanpinEvent(OrderSnap &o, string logic, int legNo, int basketId,
                      BasketState &pre, bool hasPre, double lotMult)
{
   string f[]; EvInit(f);
   datetime now = TimeCurrent();
   string sym = o.symbol;
   ...
   double pt  = MarketInfo(sym, MODE_POINT);
   if(pt <= 0) pt = Point;
   bool isBuy = (o.type == OP_BUY);

   f[EV_EVENT]     = "NANPIN";
   f[EV_TIME]      = EvTime(now);
   f[EV_LOGIC]     = logic;
   f[EV_SIDE]      = isBuy ? "buy" : "sell";
   f[EV_BASKET]    = IntegerToString(basketId);
   f[EV_LEG]       = IntegerToString(legNo);
   f[EV_PRICE]     = DoubleToString(o.openPrice, dg);
   f[EV_BID]       = DoubleToString(bid, dg);
   f[EV_ASK]       = DoubleToString(ask, dg);
   f[EV_SPREAD]    = DoubleToString((ask - bid) / pt, 1);
   f[EV_LOT_RATIO] = DoubleToString(lotMult, 3);
   if(!hasPre)
   {
      f[EV_GAP] = "no_basket";
      EvWrite(f, sym);
   ...
   double advNow = isBuy ? (pre.lastPrice - o.openPrice) / pt : (o.openPrice - pre.lastPrice) / pt;
   f[EV_PREV_LEG_PRICE] = DoubleToString(pre.lastPrice, dg);
   f[EV_PREV_LEG_TIME]  = EvTime(pre.lastLegTime);
   f[EV_SEC_PREV_LEG]   = IntegerToString((int)(o.openTime - pre.lastLegTime));
   f[EV_ADV_NOW]        = DoubleToString(advNow, 1);
   if(pre.advMaxTime > 0)
   {
      f[EV_ADV_MAX]      = DoubleToString(pre.advMaxPts, 1);
      f[EV_ADV_MAX_TIME] = EvTime(pre.advMaxTime);
      f[EV_BOUNCE]       = DoubleToString(pre.advMaxPts - advNow, 1);
   }
   if(pre.highSince > 0) f[EV_HIGH_SINCE] = DoubleToString(pre.highSince, dg);
   if(pre.lowSince  > 0) f[EV_LOW_SINCE]  = DoubleToString(pre.lowSince,  dg);
   if(pre.condMetTime > 0)
   {
      f[EV_COND_TIME] = EvTime(pre.condMetTime);
      f[EV_WAIT_SEC]  = IntegerToString((int)MathMax(0, o.openTime - pre.condMetTime));
   }
   if(pre.restored) f[EV_GAP] = "restored";
   EvWrite(f, sym);
}

// バスケットの決済: 最後のレグ以降の最大含み益(MFE)、含み益が ProbeProfitPts に達した時刻とその後の最小含み益
void WriteBasketCloseEvent(BasketState &b, double closePrice, string closeEvent)
{
   string f[]; EvInit(f);
   datetime now = TimeCurrent();
   string sym = b.symbol;
   ...
   double pt  = MarketInfo(sym, MODE_POINT);
   if(pt <= 0) pt = Point;
   bool isBuy = (b.type == OP_BUY);

   f[EV_EVENT]         = "BASKET_CLOSE";
   f[EV_TIME]          = EvTime(now);
   f[EV_LOGIC]         = b.grid;
   f[EV_SIDE]          = isBuy ? "buy" : "sell";
   f[EV_BASKET]        = IntegerToString(b.basketId);
   f[EV_LEGS]          = IntegerToString(b.maxLegs);
   f[EV_TOTAL_LOTS]    = DoubleToString(b.maxTotalLots, 2);
   f[EV_AVG_PRICE]     = DoubleToString(b.avgPrice, dg);
   f[EV_PROFIT]        = DoubleToString(b.realizedSum, 0);
   if(closePrice > 0)
   {
      f[EV_CLOSE_PRICE] = DoubleToString(closePrice, dg);
      f[EV_CLOSE_PTS]   = DoubleToString((isBuy ? closePrice - b.avgPrice : b.avgPrice - closePrice) / pt, 1);
   }
   if(b.lastLegTime > 0)   f[EV_SEC_LAST_LEG]  = IntegerToString((int)(now - b.lastLegTime));
   if(b.firstOpenTime > 0) f[EV_SEC_FIRST_LEG] = IntegerToString((int)(now - b.firstOpenTime));
   if(b.mfeTime > 0)
   {
      f[EV_MFE]             = DoubleToString(b.mfePts, 1);
      f[EV_MFE_TIME]        = EvTime(b.mfeTime);
      f[EV_SEC_MFE_TO_CLOSE]= IntegerToString((int)(now - b.mfeTime));
      f[EV_MAE]             = DoubleToString(b.maePts, 1);
   }
   if(b.probeTime > 0)
   {
      f[EV_PROBE_TIME]    = EvTime(b.probeTime);
      f[EV_MIN_FAV_PROBE] = DoubleToString(b.minFavAfterProbe, 1);
   }
   string gap = "";
   if(b.restored) gap = "restored";
   if(closeEvent == "CLOSE_NOHIST") gap = (gap == "") ? "close_nohist" : gap + ",close_nohist";
   f[EV_GAP] = gap;
   EvWrite(f, sym);

   int ki = EvKeyIndex(b.grid, b.type);
   g_evLastClose[ki] = now;
}
...
void WriteBasketSummary(BasketState &b, string sym, datetime closeTime)
{
   ...
   AppendLine(g_fnBasket, row);
   Print("ナンピン束クローズ: basket=", b.basketId, " ", b.symbol,
         " 最大段数=", b.maxLegs, " 実現損益=", DoubleToString(b.realizedSum, 2));

   if(TelegramActive() && TelegramNotifyBasketClose)
   {
      string msg = "[EA_Monitor] ナンピン束 完結\n"
                 + LogicLabel(b.grid, b.type) + "  " + b.symbol + "  最大" + IntegerToString(b.maxLegs) + "段\n"
...
void MaybeWriteEquitySnapshot()
{
   ...
   AppendLine(g_fnEquity, row);

   if(TelegramActive() && TelegramNotifyMarginAlert && margin > 0 && marginLevel < TelegramMarginAlertPct)
   {
...
{
   if(!TelegramActive()) return;
   if(IsDemoMode()) text = "[デモ " + IntegerToString(AccountNumber()) + "] " + text;
...
//| ヘルパー                                                          |
//+------------------------------------------------------------------+
// [v3.6] 通知用のロジック名。例: "ロジックb 買い"(コメントが無い注文は "買い" だけ)
string LogicLabel(string grid, int type)
{
   string dir = (type == OP_BUY || type == OP_BUYLIMIT || type == OP_BUYSTOP) ? "買い" : "売り";
   return (grid == "") ? dir : "ロジック" + grid + " " + dir;
}
...
{
   if(!MonthEndStopActive()) return IsDemoMode() ? "月末月初の停止: デモ口座のため使わない" : "月末月初の停止: 無効";
   datetime td = TradingDayJst();
   datetime ws = 0, we = 0;
   datetime prevMonthDay = FirstOfMonth(td) - 86400;
...
{
   Print("[月末月初] ", text);
   if(TelegramActive() && TelegramNotifyMonthEndStop)
...
string StopStateText(color &clr)
{
   switch(g_stopState)
   {
      ...
      case 4: clr = clrGray;      return IsDemoMode() ? "デモ口座:記録のみ" : "月末月初:無効";
...
void SendStartupNotifyOnce()
{
   ...
   string msg = "[EA_Monitor v3.8] 起動しました\n"
              + "口座: " + IntegerToString(AccountNumber()) + (IsDemoMode() ? "(デモ)" : "(リアル)") + "  " + Symbol() + "\n"
              + "ファイル名の例: " + g_fnEvent + "\n"
              + "自動売買: " + (AutoTradingIsOn() ? "ON" : "OFF")
              + "  DLL許可: " + (IsDllsAllowed() ? "あり" : "なし") + "\n"
              + NextStopWindowText();
   Print(msg);
   if(TelegramActive() && TelegramNotifyStartup) SendTelegramMessage(msg);
}
